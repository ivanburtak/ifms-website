<template>
  <section class="container-custom py-8">
    ...
    <div class="max-w-2xl">
      ...
      <p class="text-slate-600 mb-8">Приєднайтеся до нашої спільноти математиків. Заповніть форму нижче, і ми зв'яжемось
        з вами.</p>
      ...
      <form @submit.prevent="apply" class="space-y-6">
        <div>
          ...
          <input v-model="form.name" placeholder="Ваше ім'я" required
            class="w-full border border-slate-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent" />
        </div>
        ...
        <div>
          ...
          <textarea v-model="form.message" placeholder="Розкажіть про себе, вашу спеціальність та причини вступу..."
            class="w-full border border-slate-300 rounded-lg px-4 py-3 h-32 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent resize-none"></textarea>
          <p class="text-sm text-slate-500 mt-1">Мінімум 10 символів</p>
        </div>

        <button type="submit"
          class="w-full px-6 py-3 bg-sky-600 hover:bg-sky-700 text-white rounded-lg font-semibold transition duration-200 flex items-center justify-center gap-2 !text-white">
  ...
  setup() {
    ...
    const apply = async () => {
      try {
        ...
        await axios.post(`${api}/api/applications/`, form)
        form.name = ''
        form.email = ''
        form.message = ''
        ok.value = true
